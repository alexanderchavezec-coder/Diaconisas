...
async def update_visitor(visitor_id: str, visitor_input: VisitorCreate, current_user: str = Depends(get_current_user)):
    ...
    if not record:
        raise HTTPException(status_code=404, detail="Visitor not found")
    values = [visitor_id, visitor_input.nombre, visitor_input.de_donde_viene, record.get('fecha_registro', datetime.now(timezone.utc).isoformat())]
    sheets_service.update_row('Amigos', record['_row'], values)
    sheets_cache.invalidate('Amigos')
    return Visitor(id=visitor_id, nombre=visitor_input.nombre, de_donde_viene=visitor_input.de_donde_viene, fecha_registro=datetime.fromisoformat(record.get('fecha_registro', datetime.now(timezone.utc).isoformat())))
